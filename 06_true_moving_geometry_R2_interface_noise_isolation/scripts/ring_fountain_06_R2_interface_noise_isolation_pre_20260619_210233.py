...
def archive_script() -> dict[str, str]:
    ...
    for dst in [SCRIPT_CANONICAL, SCRIPT_LOCAL]:
        ...
        if dst.exists():
            ...
            shutil.copy2(src, ts)
            result[str(dst)] = f"canonical existed; current script saved as {ts}; canonical was not overwritten"
        else:
...
def extract_methods_from_arrays(case_id: str, inner: int, time_s: float, r: np.ndarray, z: np.ndarray, phi: np.ndarray) -> list[dict[str, Any]]:
    ...
    m1 = points_from_bins(r, z, phi, r_min=0.0, r_max=RTANK - 0.002, z_min=ZMIN + 0.002, z_max=ZMAX - 0.004, bins=150, exclude_ring=True)
    rows.append(metric_row_from_points(m1, case_id=case_id, inner=inner, time_s=time_s, method="M1_ROI_main_free_surface"))
    m2 = points_from_bins(r, z, phi, r_min=0.0, r_max=1.2 * RI, z_min=-0.010, z_max=0.010, bins=60, exclude_ring=True)
    rows.append(metric_row_from_points(m2, case_id=case_id, inner=inner, time_s=time_s, method="M2_center_window"))
    m3 = points_from_bins(r, z, phi, r_min=RO + 0.004, r_max=RTANK - 0.004, z_min=-0.010, z_max=0.010, bins=90, exclude_ring=True)
...
def render_points_check(path: Path, case_id: str, rows: list[dict[str, Any]], model: Any, inner: int) -> None:
    ...
    roi = points_from_bins(r, z, phi, r_min=0.0, r_max=RTANK - 0.002, z_min=ZMIN + 0.002, z_max=ZMAX - 0.004, bins=150, exclude_ring=True)
