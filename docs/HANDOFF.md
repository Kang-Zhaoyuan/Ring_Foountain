...
- The workbook labels `5.2/12.6 mm` as inner/outer diameter, thickness `5.02 mm`, mass `5.35 g`, and release height `100+/-5 (105)`. Literal rectangular geometry implies `10.302 g/cm3`, conflicting with the earlier `7.8 g/cm3` value.
- A provisional literal-geometry L7 run used measured mass directly and `105 mm` release height. It completed uninterrupted to 340 ms with `0.1123%` maximum budget residual, `9.607 m/s` maximum laboratory speed, zero invalid values, and zero deep-solid liquid.
...
- The run is under-resolved: only `3.95` L7 cells span the provisional radial metal width. No contact-angle or surface-tension fit was attempted.
...
- Start-of-round Git HEAD was `a062162` (`Constrain L7 cavity detachment with experiment`). The current Codex sandbox mounts `.git` read-only: `git add` failed when creating `.git/index.lock`, so this round is not committed. This is a sandbox permission, not a repository-content failure. Push remains manual by project rule and user choice.
...
The only next action is to freeze the specimen metadata: confirm whether
`5.2/12.6 mm` are diameters, whether `5.35 g` is the same solid ring, and
whether `(105)` is the actual release height. Only after that confirmation
should the literal specimen be rerun at L8 and selected raw frames be used to
calibrate the coherent-height image criterion.
