...
17. First project-owned dynamic-wetting ring-entry implementation: case 15 uses the 26.15 g specimen and prescribed video trajectory, replaces the one-time depth event with a finite-speed, source-free ghost-cell wetting front, and records connected-jet and conservation diagnostics. The first L7 run completes delayed detachment but still misses the thin experimental jet; external reproduction is no longer a gate.
18. Case 16 freezes a through-hole-specific PLIC/connectivity metric, aperture flux and pressure diagnostics. It reproduces the Case 15 52.5 ms centre height (`6.7173 mm`), passes no-ring and closed-disk controls, and finds that contact-triggered and instant inner ghost-cell wetting do not change the flow. No candidate passes the L8 gate; the impact-stage jet remains `98.65--99.08 mm` below experiment.
19. Case 17 adds an explicitly hybrid, experiment-calibrated visible-tip model while retaining an auditable L7 bulk VOF calculation. The hybrid height is `105.80 mm` at 52.5 ms, later first/Worthington checkpoints pass their provisional pixel uncertainties, and the 165 ms bulk run exits normally with zero invalid values. Native PLIC remains reported separately and is not presented as a resolved 105.8 mm filament.
