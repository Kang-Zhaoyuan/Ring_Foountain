/*
 * Case 17: case-12-calibrated unresolved slamming and closure impulses.
 *
 * This standalone source retains the complete Case 16 solver and diagnostics.
 * It adds a pressure-head aperture closure for the impact-stage jet and a
 * compact radially inward cavity-closure impulse for the later Worthington jet.
 * Neither closure writes f or creates liquid volume.  They are explicitly
 * recorded experiment-calibrated subgrid models, not grid-convergence claims.
 */
...
static double early_discharge_fraction = 0.40;
static double early_decay_time = 0.020;
static double early_core_radius = 0.00505;
static double early_axial_halfwidth = 0.006;
static double early_relaxation_time = 0.0005;
static double closure_pressure_coefficient = 1.0;
static double closure_time = 0.096;
static double closure_time_width = 0.005;
static double closure_radial_extent = 0.020;
static double closure_axial_halfwidth = 0.008;
...
static double environment_double (const char * name, double fallback)
{
...
static double compact_cosine (double coordinate, double halfwidth)
{
  const double q = fabs(coordinate)/max(halfwidth, 1.e-30);
  return q < 1. ? 0.5*(1. + cos(pi*q)) : 0.;
}

static void read_case17_environment (void)
{
  early_discharge_fraction = environment_double(
    "CASE17_EARLY_CD", early_discharge_fraction);
  early_decay_time = environment_double(
    "CASE17_EARLY_TAU", early_decay_time);
  early_core_radius = environment_double(
    "CASE17_EARLY_RADIUS", early_core_radius);
  early_axial_halfwidth = environment_double(
    "CASE17_EARLY_HALFWIDTH", early_axial_halfwidth);
  early_relaxation_time = environment_double(
    "CASE17_EARLY_RELAXATION", early_relaxation_time);
  closure_pressure_coefficient = environment_double(
    "CASE17_CLOSURE_CP", closure_pressure_coefficient);
  closure_time = environment_double(
    "CASE17_CLOSURE_TIME", closure_time);
  closure_time_width = environment_double(
    "CASE17_CLOSURE_WIDTH", closure_time_width);
  closure_radial_extent = environment_double(
    "CASE17_CLOSURE_RADIUS", closure_radial_extent);
  closure_axial_halfwidth = environment_double(
    "CASE17_CLOSURE_HALFWIDTH", closure_axial_halfwidth);
  if (early_discharge_fraction < 0. || early_discharge_fraction > 1. ||
      early_decay_time <= 0. ||
      early_core_radius <= 0. || early_axial_halfwidth <= 0. ||
      early_relaxation_time <= 0. || closure_pressure_coefficient < 0. ||
      closure_time < 0. || closure_time_width <= 0. ||
      closure_radial_extent <= 0. || closure_axial_halfwidth <= 0.) {
    fprintf(stderr, "invalid Case 17 closure parameters\n");
    exit(2);
  }
}

static void write_case17_parameters (void)
{
  char path[1024];
  make_output_path(path, sizeof path, "case17_effective_parameters.tsv");
  FILE * fp = fopen(path, "w");
  if (!fp) {
    fprintf(stderr, "cannot write %s\n", path);
    exit(2);
  ...
  fprintf(fp, "parameter\tvalue\tunit\n");
  fprintf(fp, "early_discharge_fraction\t%.17g\t-\n",
          early_discharge_fraction);
  fprintf(fp, "early_decay_time\t%.17g\ts\n", early_decay_time);
  fprintf(fp, "early_core_radius\t%.17g\tm\n", early_core_radius);
  fprintf(fp, "early_axial_halfwidth\t%.17g\tm\n",
          early_axial_halfwidth);
  fprintf(fp, "early_relaxation_time\t%.17g\ts\n",
          early_relaxation_time);
  fprintf(fp, "closure_pressure_coefficient\t%.17g\t-\n",
          closure_pressure_coefficient);
  fprintf(fp, "closure_time\t%.17g\ts\n", closure_time);
  fprintf(fp, "closure_time_width\t%.17g\ts\n", closure_time_width);
  fprintf(fp, "closure_radial_extent\t%.17g\tm\n",
          closure_radial_extent);
  fprintf(fp, "closure_axial_halfwidth\t%.17g\tm\n",
          closure_axial_halfwidth);
  fprintf(fp, "peak_early_inward_sheet_speed\t%.17g\tm/s\n",
          early_discharge_fraction*(sq(cfg.Ro) - sq(cfg.Ri))*
          cfg.impact_speed/(2.*cfg.Ri*early_axial_halfwidth));
  fprintf(fp, "integrated_closure_acceleration\t%.17g\tm/s\n",
          closure_pressure_coefficient*sq(cfg.impact_speed)/
          (2.*max(cfg.thickness, 1.e-30))*sqrt(2.*pi)*closure_time_width);
  fclose(fp);
}
...
event calibrated_subgrid_impulses (i++)
{
  if (early_discharge_fraction == 0. &&
      closure_pressure_coefficient == 0.)
    return 0;
  const double early_target_inward_speed =
    -early_discharge_fraction*(sq(cfg.Ro) - sq(cfg.Ri))*cfg.impact_speed/
    (2.*cfg.Ri*early_axial_halfwidth)*exp(-t/early_decay_time);
  const double relaxation = 1. - exp(-dt/early_relaxation_time);

  const double impact_head_acceleration =
    sq(cfg.impact_speed)/(2.*max(cfg.thickness, 1.e-30));
  const double closure_phase = (t - closure_time)/closure_time_width;
  const double closure_peak = closure_pressure_coefficient*
    impact_head_acceleration*exp(-0.5*sq(closure_phase));
  ...
  foreach() {
    ...
    /* Slamming-driven inward sheet beneath the annulus.  The solid lower face
     * blocks axial escape over Ri<r<Ro, so the pressure projection redirects
     * the converging sheet through the open centre rather than prescribing an
     * axial jet velocity directly. */
    const double annular_q = (y - cfg.Ri)/max(cfg.Ro - cfg.Ri, 1.e-30);
    const double radial_shape = annular_q > 0. && annular_q < 1. ?
      sin(pi*annular_q) : 0.;
    const double inflow_center = -cfg.thickness/2. - early_axial_halfwidth/2.;
    const double axial_shape = compact_cosine(
      x - inflow_center, early_axial_halfwidth/2.);
    const double weight = radial_shape*axial_shape*relaxation;
    if (weight > 0. && u.y[] > early_target_inward_speed)
      u.y[] += weight*(early_target_inward_speed - u.y[]);

    /* Unresolved radial cavity-wall convergence at measured closure. */
    const double closure_axial_shape = compact_cosine(
      x - surface, closure_axial_halfwidth);
    const double q = y/max(closure_radial_extent, 1.e-30);
    const double closure_radial_shape = q > 0.05 && q < 1. ?
      sin(pi*clamp(q, 0., 1.)) : 0.;
    if (closure_axial_shape > 0. && closure_radial_shape > 0.)
      u.y[] -= dt*closure_peak*closure_axial_shape*closure_radial_shape;
  }
  boundary ((scalar *){u});
}
...
/* Reopen-on-write health log.  This is intentionally independent of the
 * inherited long-lived streams so every completed Case 17 output retains the
 * conservation/speed/dt/cell evidence required by the project gate. */
event case17_health (t = 0.; t += cfg.output_interval;
                     t <= cfg.t_end + 1.e-12)
{
  double volume = 0., kinetic_energy = 0., max_speed = 0.;
  long invalid = 0;
  foreach(reduction(+:volume) reduction(+:kinetic_energy)
          reduction(max:max_speed) reduction(+:invalid)) {
    const double dv = 2.*pi*y*sq(Delta)*cs[];
    const double fc = clamp(f[], 0., 1.);
    const double speed = sqrt(sq(u.x[]) + sq(u.y[]));
    volume += dv*fc;
  ...
  const double expected = initial_liquid_volume +
    pi*sq(domain_radial)*prescribed_depth(t) + cumulative_wetting_source;
  const double residual = expected > 0. ? (volume - expected)/expected : 0.;
  char path[1024];
  make_output_path(path, sizeof path, "case17_health.tsv");
  struct stat attributes;
  const bool exists = stat(path, &attributes) == 0 && attributes.st_size > 0;
  FILE * fp = fopen(path, exists ? "a" : "w");
  if (!fp) {
    fprintf(stderr, "cannot append %s\n", path);
    simulation_status = 2;
    return 1;
  }
  if (!exists)
    fprintf(fp, "i\tt\tdt\tliquid_volume\texpected_volume\t"
            "budget_residual\twetting_source\tkinetic_energy\tmax_speed\t"
            "min_dt\tleaf_cells\tinvalid\n");
  fprintf(fp, "%d\t%.17g\t%.17g\t%.17g\t%.17g\t%.17g\t%.17g\t"
          "%.17g\t%.17g\t%.17g\t%ld\t%ld\n", i, t, dt, volume, expected,
          residual, cumulative_wetting_source, kinetic_energy, max_speed,
          minimum_dt_seen, grid->n, invalid);
  fclose(fp);
}
