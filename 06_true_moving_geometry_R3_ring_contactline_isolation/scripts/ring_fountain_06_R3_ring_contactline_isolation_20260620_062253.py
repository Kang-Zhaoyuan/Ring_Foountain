...
def phase2(client: Any) -> dict[str, Any]:
    ...
    controls_for_phase2 = read_csv(R2_DIR / "02_static_control_decomposition" / "tables" / "static_control_cases.csv")
    for cid, r2_case in [("C1", "C1_static_no_ALE"), ("C2", "C2_static_ALE_present_zero_displacement"), ("C3", "C3_static_no_ring_rectangular_seed")]:
        match = next((r for r in controls_for_phase2 if r.get("case_id") == r2_case), {})
        if not match:
            summary_rows.append({"case_id": cid, "status": "missing_R2_control_row", "principal_spike_region": "unresolved"})
            continue
        rough = match.get("interface_roughness_final", math.nan)
        pflag = str(match.get("pseudo_spike_ROI_flag")).lower() == "true"
        principal = "global" if pflag else "none_clear"
        summary_rows.append(
            {
                "case_id": cid,
                "status": "reused_R2_control_csv_due_memory_guard",
                "roughness_peak_to_peak": rough,
                "pseudo_spike_regional_flag": pflag,
                "principal_spike_region": principal,
                "ring_present": match.get("ring_present"),
                "ALE_present": match.get("ALE_present"),
                "case_pass": match.get("case_pass"),
                "note": "R3 avoids heavy reload of R2 control models after ComsolUI/memory issues; Phase 3 performs fresh geometry controls.",
            }
        )
        rows.append(
            {
                ...
                "roughness_peak_to_peak": rough,
                "pseudo_spike_regional_flag": pflag,
                "contamination_flags": "memory_guard_reused_R2_control_csv",
            }
        )
    ...
    principal = "global" if flagged else "none_clear"
    report = [
    ...
    log("Phase 2 done with memory guard")
    return {"status": "PASS_WITH_MEMORY_GUARD", "ALLOW_PHASE3": "YES", "principal_spike_region": principal, "rows": summary_rows}
...
def phase3(client: Any) -> dict[str, Any]:
    log("Phase 3 start")
    existing_csv = R3 / "03_ring_geometry_position_controls" / "tables" / "ring_geometry_position_cases.csv"
    if existing_csv.exists():
        existing_rows = read_csv(existing_csv)
        if any("MemoryError" in str(r.get("failure_message", "")) for r in existing_rows if r.get("case_id") in {"G2_ring_deeper_submerged", "G3_ring_far_below_surface"}):
            report = [
            ...
            log("Phase 3 reused existing MemoryError rows and stopped by diagnostic memory guard")
            return {
                "status": "FAIL_DIAGNOSTIC_MEMORY_GUARD",
                "ALLOW_PHASE8": "NO",
                "ALLOW_PHASE4": "NO",
                "STOP_AFTER_PHASE3": "YES",
                "conclusion": "diagnostic_postprocessing_memory_blocker",
                "best_case": {},
                "rows": existing_rows,
            }
    r2_controls = read_csv(R2_DIR / "02_static_control_decomposition" / "tables" / "static_control_cases.csv")
    c3 = next((r for r in r2_controls if r.get("case_id") == "C3_static_no_ring_rectangular_seed"), {})
    c2 = next((r for r in r2_controls if r.get("case_id") == "C2_static_ALE_present_zero_displacement"), {})
    rows: list[dict[str, Any]] = [
        {
            "case_id": "G0_no_ring_rectangular_repeat",
            "geometry_change": "no ring; reused R2 C3 equivalent control",
            "solve_status": c3.get("solve_status", "REUSED"),
            "H_final_minus_H0": c3.get("H_final_minus_H0"),
            "regional_roughness": c3.get("interface_roughness_final"),
            "pseudo_spike_regional_flag": str(c3.get("pseudo_spike_ROI_flag")).lower() == "true",
            "interface_quality": c3.get("interface_quality"),
            "case_pass": str(c3.get("case_pass")).lower() == "true",
            "note": "R2 C3 no-ring seed reused to avoid redundant long solve.",
        },
        {
            "case_id": "G1_ring_current_position_repeat",
            "geometry_change": "current ring; reused R2 C2 equivalent static ALE control",
            "solve_status": c2.get("solve_status", "REUSED"),
            "H_final_minus_H0": c2.get("H_final_minus_H0"),
            "regional_roughness": c2.get("interface_roughness_final"),
            "pseudo_spike_regional_flag": str(c2.get("pseudo_spike_ROI_flag")).lower() == "true",
            "interface_quality": c2.get("interface_quality"),
            "case_pass": str(c2.get("case_pass")).lower() == "true",
            "note": "R2 C2 current ring static ALE control reused to avoid redundant long solve.",
        },
    ]
    cases = [
    ...
    for case in cases:
        ...
        rows.append(row)
        write_csv(R3 / "03_ring_geometry_position_controls" / "tables" / "ring_geometry_position_cases.csv", rows)
        if case["case_id"] in {"G2_ring_deeper_submerged", "G3_ring_far_below_surface"} and row.get("case_pass"):
            rows.append({"case_id": "G4_ring_above_surface_air_side", "solve_status": "SKIP", "case_pass": False, "failure_message": "Skipped because submerged-position control already produced a clear ring-present baseline candidate."})
            rows.append({"case_id": "G5_ring_thinner_or_smaller_gap_control", "solve_status": "SKIP", "case_pass": False, "failure_message": "Skipped because submerged-position control already produced a clear ring-present baseline candidate."})
            write_csv(R3 / "03_ring_geometry_position_controls" / "tables" / "ring_geometry_position_cases.csv", rows)
            break
        if case["case_id"] == "G3_ring_far_below_surface" and not any(r.get("case_pass") for r in rows if str(r.get("case_id", "")).startswith("G") and r.get("case_id") not in {"G0_no_ring_rectangular_repeat"}):
            rows.append({"case_id": "G4_ring_above_surface_air_side", "solve_status": "SKIP", "case_pass": False, "failure_message": "Skipped after current/deeper/far-below ring controls showed same failure type; switching to WettedWall/contactline root cause."})
            rows.append({"case_id": "G5_ring_thinner_or_smaller_gap_control", "solve_status": "SKIP", "case_pass": False, "failure_message": "Skipped after current/deeper/far-below ring controls showed same failure type; switching to WettedWall/contactline root cause."})
            write_csv(R3 / "03_ring_geometry_position_controls" / "tables" / "ring_geometry_position_cases.csv", rows)
            break
    write_csv(R3 / "03_ring_geometry_position_controls" / "tables" / "ring_geometry_position_cases.csv", rows)
    render_bars(
...
def main() -> int:
    ...
    try:
        ...
        elif summary["Phase3"].get("STOP_AFTER_PHASE3") == "YES":
            summary["ring_position_cause"] = summary["Phase3"].get("conclusion")
            summary["diagnostic_postprocessing_blocker"] = "Python MemoryError while evaluating regional interface metrics on solved G2/G3 models"
        else:
